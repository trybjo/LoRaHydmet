...
char HWMessage[] = "Hello World ! I'm happy if you can read me2";
...
void loop()
{
  SendEncrypted(HWMessage);
  delay(6000);
}


void SendEncrypted(char message[]){
  int messageLen = strlen(message);
  uint8_t data[messageLen+1] = {0};
  for(uint8_t i = 0; i<= messageLen; i++) data[i] = (uint8_t)message[i];
